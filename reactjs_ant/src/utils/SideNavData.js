...
import { roleName } from "config/const/roleName";
import { ClusterOutlined } from "@ant-design/icons";

const itemsAdmin = [
  {
    label: (pathname, color) =>
      getItem(
        "Danh sách Công ty",
        "/list-GP-company",
        <ClusterOutlined />,
...
export const items = (pathname, color) => {
  ...
  return [
    ...
    ...items.map((element, index) => ({
      key: index + 200,
      label: element.label(pathname, color),
    })),
    // {
    //   key: "3",
    //   label: getItem(
    ...
    {
      key: "5",
      label: getItem("Tài khoản", "/profile", profileSVg, pathname, color),
    },
  ];
};
